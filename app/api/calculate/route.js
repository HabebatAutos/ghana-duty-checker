...
async function fetchMsrpLineup(year, make, model, origin, userEngine = '', userBodyType = '', isBackgroundSync = false, originCode = '') {
  ...
  // 1. AUTOMATED LOCAL SPREADSHEET SCANNER (STRICT LOCAL DATABASE FIRST)
  try {
    ...
    if (targetPresetFile) {
      ...
      // Filter 2: Strict/Resilient Local Database Origin Matching
  ...
  // 4. LIVE LLM HOOK (Strictly isolated fallback when local database has zero records)
